...
so low that we don't expect to clip any entries we can use $\overline{f}$ as an approximation of $f$.
...
\end{equation*}
due to Corollary \ref{cor:expNorm}. We then relax this constraint by omitting $\| \bm{\mu} - \mue \|^2$, 
...
Now to evaluate the expected error of the mechanism, which is given by equation \eqref{eq:Alg3Err}, it is highly important to determine the value $C$.
...
For the sake of analysis, I will provide an upper bound on $C$ using Bernstein's inequality.
...
$n$ is the number of points which is needed to decide the clipping probability.
