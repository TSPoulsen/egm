...
works very similairly to the Gaussian Mechanism as described by algorithm \ref{alg:gaussmech}, though instead of sampling $\eta_j$ from 
$\mathcal{N}(0,\sigma_{\epsilon, \delta}^2)$, here it is instead drawn from $\mathcal{N}\lr{0,\lr{\sigma_{opt}\cdot \frac{\Delta_j}{b_j}}^2}$. 
...
\todo{add lemma/theorem/proof of why this is \edp (figure out if needed). Prolly is as the method is much the same for gaussian data}
...
in \cite{Lebeda2022}, and the expected error of the mechanism.

\begin{theorem}[Optimality and error of the Elliptical Gaussian Mechanism \cite{Lebeda2022}]
The value for $b_j$ which minimized expected $l_2$ error $\ee{\eta}$ of the Elliptical Gaussian Mechanism is as follows
\[
...
\begin{equation}
\label{eq:EGMErr}
    \ee{\eta} =  \sigma_{opt} \cdot \| \bm{\Delta} \|_1^2
\end{equation}
...
\[
    \frac{d \cdot \sigma_{opt}^2 \cdot \| \bm{\Delta} \|^2 }{\sigma_{opt} \cdot \| \bm{\Delta} \|_1^2} = \lr{\frac{\sqrt{d}\| \bm{\Delta} \|}{\| \bm{\Delta} \|_1}}^2
\]
...
It is quite apparent that determining a $ \Delta_j $ is impossible 
...
\]
The intuition in this context is the same as that for the Elliptical Gaussian Mechanism, that dimensions with high variance should have more noise
added. Therefore I will use the same approach as that used by the Elliptical Gaussian Mechanism to achieve \edp.
...
There does not exist such a transformation which is linear, as some points will always lie outside the unit ball with at least a small probability. 
Instead I will introduce the constraint that the expected norm of vectors after the transformation is $1$. 
I whish to find a scaling of each dimension $b_j$ s.t.
\todo{Introduce $\sigma_j$ before here}
\begin{align}
\label{eq:Alg3Con}
...
The transformation back to the original space is the done by multiplying each dimension with $b_j^{-1}$, and due to the linearity of transformation 
...
\]
\todo{Explain that multiplying Gaussian with factor is multiplying std with factor (perhaps make lemma)}
in which all $\eta_j \sim \mathcal{N}(0,(2C\sigma_{opt})^2)$, and then by Lemma \ref{lem:GaussTransform} we have $b_j^{-1}\eta_j \sim \mathcal{N}(0, (b_j^{-1} \cdot 2C\sigma_{opt})^2)$.
...
\begin{lemma}
\label{lem:Optb}
Let $X \in \R^{n \times d}$ be a dataset where each dimension $X^{(j)}$ is independently gaussianly distributed, i.e.
$X^{(j)} \sim \mathcal{N}(\mu_j, \sigma_j^2)$. Then the expected error of Algorithm \ref{alg:EGM2} is minimized when
\[
...
Some have also studied giving tail bounds on these probabilities \cite{QaudFormsBounds}, however there was none which were of use in this setting.
...
\todo{Give Bernsteins inequality}
...
\[
0 \le t \le 
    \frac{1}{4 \sigma_*^2} \sqrt{2 \sum_{j \in [d]} \sigma_j^4}
\]
...
\begin{align*}
    ...
    \sigma_j^{2k} \cdot \prod_{c=2}^k (2c-1) \\
    &\le \sigma_j^{2k} \cdot \prod_{c=2}^k 2c
    = \sigma_j^{2k} \cdot 2^{k-1} \cdot \frac{k!}{2}
    = \sigma_j^{2k} \cdot 2^{k-2} \cdot k!
\end{align*}
Concluding that $\Exp{ |Y_j^k| } \le  \sigma_j^{2k} \cdot 2^{k-2} \cdot k!$.
...
\begin{align*}
    &\sigma_j^{2k} \cdot 2^{k-2} \cdot k! \le \frac{1}{2} \cdot 2 \sigma_j^4 L^{k-2} k! \iff \\
    &\sigma_j^{2k-4} \cdot 2^{k-2} \cdot \le L^{k-2} \iff \\
    &\sigma_j^2 \cdot 2 \le L
\end{align*}
As this has to hold for all $j \in [d]$, we can simply choose
$ L := 2 \cdot \max_{j \in [d]} \sigma_j^2$ which satisfies the constraint.
...
\[
    ...
    \frac{1}{4 \cdot \max_{j \in [d]} \sigma_j^2} \sqrt{2 \sum_{j \in [d]} \sigma_j^4}
\]
\end{proof}
...
\noindent When $\ln (n) \le \frac{\sum_{j \in [d]} \sigma_j^2}{8\max_{j \in [d]} \sigma_j^2}$
...
\begin{align*}
    &0 \le t \le \frac{1}{4 \max_{j \in [d]}\sigma_j^2} \sqrt{2\sum_{j \in [d]} \sigma_j^4} \iff \\
    &\sqrt{\ln (n)} \le \sqrt{\lr{\frac{\sum_{j \in [d]} \sigma_j}{\max_{j \in [d]} \sigma_j}}^2 \cdot \frac{1}{8} \cdot \sum_{j \in [d]} \lr{\frac{\sigma_j}{\sum_{i \in [d]} \sigma_i}}^2} \iff \\
    &\ln (n) \le \frac{1}{8} \frac{1}{\max_{j \in [d]} \sigma_j^2} \cdot \sum_{j \in [d]} \sigma_j^2 \iff \\
    &\ln (n) \le \frac{\sum_{j \in [d]} \sigma_j^2}{8\max_{j \in [d]} \sigma_j^2}
\end{align*}
