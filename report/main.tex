...
Comparing the expected error between Algorithm \ref{alg:gaussmech} and Algorithm \ref{alg:EllipGM} it can be seen 
that the error for the Elliptical Gaussian Mechanism is often smaller. If all $\Delta_j \ge 1$ then the following ration must be greater than $1$
\[
    \frac{d \cdot \sigma_{opt}^2 \cdot \| \bm{\Delta} \|^2 }{\sigma_{opt} \cdot \| \bm{\Delta} \|_1^2} = \lr{\frac{\sqrt{d}\| \bm{\Delta} \|}{\| \bm{\Delta} \|_1}}^2 \ge 1
\]
This implies that the expected error of the Elliptical Gaussian Mechanism is less than that of the Gaussian Mechanism.
%(equation \ref{eq:EGMErr})
%to that of the normal Gaussian mechanism (equation \ref{eq:GMErr}) it is apparent that the first 
