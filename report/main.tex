...
and as long as $\sigma_* \le \frac{2}{3}$ we can bound the probability for the sum of variables squared as follows 
\[
...
Equation \ref{eq:BernCon} is therefore rewritten to 
\begin{align*}
    &\frac{3}{2}\sigma_j^{2k} \cdot 2^{k-2} \cdot k! \le \frac{1}{2} \cdot 2 \sigma_j^4 L^{k-2} k! \iff \\
    &\frac{3}{2} \sigma_j^{2k-4} \cdot 2^{k-2} \le L^{k-2} \iff \\
    &\begin{cases}
        \frac{3}{2} \sigma_j \le 1 & \text{when $k=2$} \\
        \lr{\frac{3}{2}}^{\frac{1}{k-2}} \sigma_j^2 \cdot 2 \le L & \text{when $k>2$}
    \end{cases}
\end{align*}
...
\prob{\sum_{j \in [d]} X_j^2  \ge 
    ...
    \sum_{j \in [d]} \sigma_j^2 } < e^{-t^2}
\]
as long as $\sigma_* \le \frac{2}{3}$, where t must lie within
\[
...
Let $\sigma_* := \max_{j \in [d]} \sigma_j$ be the maximal standard deviation.
When $\ln (n) \le \frac{\sum_{j \in [d]} \sigma_j^2}{18 \sigma_*^2}$
    
and $\frac{\sigma_*}{\sum_{j \in [d]} \sigma_j} \le \frac{4}{9}$
the expected error of algorithm 3 can be upper bounded by 
\[
