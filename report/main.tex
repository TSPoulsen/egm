...
can then be applied to each dimension of $f(\widehat{X})$ as well as $\eta$, giving a differentially private estimate of $f(X)$, 
...
\end{theorem}
The proof will be ommitted here, but it is very similair to the proofs for lemma \ref{lem:Optb} and theorem \ref{theo:Alg3OptErr}.
...
\end{equation}
\todo{I would like to cite where I got the Bernstein Inequality from, around here. Haven't found any good source (except for wikipedia)}
...
lemma \ref{lem:Bernstein}, and inserting that $\widehat{\sigma}_j := \sigma_j \cdot b_j = \sqrt{\frac{\sigma_j}{\sum_{i \in [d]} \sigma_i}}$ 
when $b_i$ is given by theorem \ref{lem:Optb}. I wish the clipping 
probability to be less than $n^{-1}$, giving me
\[
    e^{-t^2} = \frac{1}{n} \implies t = \sqrt{\ln (n)}
\]
Combining these results I get that
\begin{align*}
    C^2 &\le 2 \sqrt{\ln(n)} \sqrt{2\sum_{j \in [d]}  \lr{\widehat{\sigma}_j^4}} + 
    \sum_{j \in [d]} \widehat{\sigma}_j^2 \\
    &=\sqrt{8 \ln(n) \sum_{j \in [d]}  \lr{\widehat{\sigma}_j^4}} + 
    \sum_{j \in [d]} \widehat{\sigma}_j^2 \\
